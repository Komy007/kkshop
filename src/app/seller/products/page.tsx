...
export default function SellerProductsPage() {
    ...
    const load = useCallback(async (p = 1, q = search, t = tab) => {
        ...
        if (t !== 'ALL') params.set('approvalStatus', t);
        ...
        <div className="max-w-5xl mx-auto py-7 px-4">
            ...
            <div className="flex items-start justify-between mb-5 gap-3">
                <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">내 상품 — {total} items total</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => load(page, search, tab)} title="Refresh · 새로고침"
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl border gap-3">
                    ...
                    <button onClick={() => load(1)} className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-bold hover:bg-teal-700">
                        Retry · 다시 시도
                    </button>
                </div>
            ) : products.length === 0 ? (
                                            ...
                                            <td className="py-3 px-4">
                                                ...
                                                {p.approvalStatus === 'REJECTED' && (
                                                    <div className="mt-1.5 p-2 bg-red-50 rounded-lg border border-red-100">
                                                        <p className="text-xs font-bold text-red-600">❌ Rejected · 반려됨</p>
                                                        {p.rejectionReason ? (
                                                            <p className="text-xs text-red-500 mt-0.5">
                                                                <span className="font-semibold">Reason · 사유:</span> {p.rejectionReason}
                                                            </p>
                                                        ) : (
                                                            <p className="text-xs text-red-400 mt-0.5">
                                                                Please edit and re-submit · 수정 후 재등록하세요
                                                            </p>
                                                        )}
                                                    </div>
                                                )}
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between mt-4">
                            <p className="text-sm text-gray-500">
                                ...
                                <span className="text-xs text-gray-400 ml-1">· {total}개</span>
                            </p>
                            <div className="flex items-center gap-1">
                                <button onClick={() => { const np = Math.max(1, page - 1); load(np, search, tab); }}
                                    disabled={page === 1}
                                ...
                                <button onClick={() => { const np = Math.min(totalPages, page + 1); load(np, search, tab); }}
