        ...
        <div className="p-6 space-y-6 max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">My Payouts &amp; Earnings</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
                        {[['7', '7일'], ['30', '30일'], ['90', '3개월'], ['365', '1년']].map(([v, l]) => (
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        { label: '총 매출',    value: `$${summary.grossRevenue.toFixed(2)}`,   icon: TrendingUp, color: 'text-blue-600',   bg: 'bg-blue-50' },
                        { label: '수수료',     value: `-$${summary.commission.toFixed(2)}`,    icon: DollarSign, color: 'text-red-500',    bg: 'bg-red-50', sub: `${summary.commissionRate}%` },
                        { label: '순 정산액',  value: `$${summary.netPayout.toFixed(2)}`,      icon: DollarSign, color: 'text-green-600',  bg: 'bg-green-50' },
                        { label: '주문 건수',  value: `${summary.orderCount}건`,               icon: ShoppingCart, color: 'text-slate-700', bg: 'bg-slate-50' },
                    ].map(card => (
                        <div key={card.label} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                            ...
                            <div className="text-xs text-slate-500 mt-1">{card.label}{(card as any).sub ? ` (${(card as any).sub})` : ''}</div>
                        </div>
                    ))}
                </div>
            ) : null}
            ...
            {summary && (
                <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 text-sm text-blue-800">
                    <span className="font-bold">정산 계산식: </span>
                    총 매출 ${summary.grossRevenue.toFixed(2)} − 수수료 {summary.commissionRate}% (${summary.commission.toFixed(2)}) = <strong>순 정산액 ${summary.netPayout.toFixed(2)}</strong>
                    <span className="text-xs opacity-70 block mt-1">* 배송완료(DELIVERED) 주문만 집계됩니다. 실제 정산은 플랫폼 관리자가 처리합니다.</span>
                </div>
            )}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-bold text-slate-700">상세 내역</h2>
                    <span className="text-xs text-slate-400">최근 {period}일 · 배송완료 주문</span>
                </div>

                {loading ? (
                    <div className="py-16 text-center text-slate-400 text-sm">계산 중...</div>
                ) : items.length === 0 ? (
                    <div className="py-16 text-center text-slate-400 text-sm">이 기간에 배송 완료된 주문이 없습니다.</div>
                ) : (
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {items.map((item, i) => (
                                    <tr key={i} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-slate-500 text-xs">{new Date(item.orderDate).toLocaleDateString('ko-KR')}</td>
