...
export default function RolesSecurityPage() {
    ...
    const fetchAdmins = async () => {
        try {
            ...
            // customers API returns { customers: [], total, page, pageSize }
            const list: AdminUser[] = Array.isArray(json) ? json : (json.customers ?? []);
            // Filter only ADMIN, SUPERADMIN, and SUPPLIER roles
            const adminUsers = list.filter((user: AdminUser) => ['ADMIN', 'SUPERADMIN', 'SUPPLIER'].includes(user.role));
